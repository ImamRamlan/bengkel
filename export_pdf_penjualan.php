...
$sql = "SELECT p.id_penjualan, p.tanggal_penjualan, p.jumlah_penjualan, p.total_biaya, s.nama_suku_cadang
        FROM penjualan p
        JOIN suku_cadang s ON p.id_suku_cadang = s.id_suku_cadang";

if ($tanggal_awal && $tanggal_akhir) {
    $sql .= " WHERE p.tanggal_penjualan BETWEEN ? AND ?";
}
...
$pdf->Cell(0, 10, 'Laporan Penjualan Suku Cadang', 0, 1, 'C');
...
$pdf->Cell(30, 8, 'ID Penjualan', 1, 0, 'C', 1);
$pdf->Cell(40, 8, 'Tanggal Penjualan', 1, 0, 'C', 1);
$pdf->Cell(50, 8, 'Nama Suku Cadang', 1, 0, 'C', 1);
...
while ($row = $result->fetch_assoc()) {
    ...
    $pdf->Cell(30, 8, $row['id_penjualan'], 1, 0, 'C');
    $pdf->Cell(40, 8, $row['tanggal_penjualan'], 1, 0, 'C');
    $pdf->Cell(50, 8, $row['nama_suku_cadang'], 1, 0, 'L');
    $pdf->Cell(30, 8, $row['jumlah_penjualan'], 1, 0, 'C');
